...
plots <- component_proximity_per_marker(es_data,
...
In the heatmaps below, proteins are selected by abundance so that the most highly expressed markers are shown.
Up to 40 proteins with the highest mean abundance are included. **Global** heatmaps use the same proteins in every
sample, ranked by mean abundance across samples. **Per Celltype** heatmaps repeat the selection within each cell type,
so each heatmap shows the most abundant proteins in that population.
